import type { AttemptFeedback, TrainerSettings, TypingMetrics } from '@/types'
import type { FieldResult, Task2ScoreResult } from './scoring'
...
export function buildTask1Feedback(
  ...
  if (metrics.completionPercentage < 100) {
    improvements.push(
      `Only ${Math.round(metrics.completionPercentage)}% of the passage was completed before time expired`,
    )
  }
