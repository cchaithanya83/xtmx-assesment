 ...
 * A login identity.
 *
 * Candidates self-register (sign-up) and get an account linked to their
 * `Candidate` record. Trainer and admin accounts are never self-registered —
 * an admin creates and manages them from the User Management screen.
 *
 * Only the PBKDF2 hash and its salt are stored; the password itself never is.
 */
export interface Account {
  ...
  passwordHash: string
  passwordSalt: string
  /** Set for candidate accounts — links to the `Candidate` profile. */
  candidateId?: string
  /** Account id of the admin who created this account (staff accounts). */
  createdBy?: string
  createdAt: string
